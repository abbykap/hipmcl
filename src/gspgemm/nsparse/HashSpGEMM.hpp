            ...
            case 6 :
                {
            	    ...
            	    fail_count = 0;
            	    cudaMalloc((void **)&d_fail_count, sizeof(idType));
				...
				idType bin_size)
{
    ...
    for (j = d_arpt[rid] + wid; j < d_arpt[rid + 1]; j += wnum) {
		...
		// acolids = d_acolids[j]; // @OGUZ-BEWARE
		// avalues = d_avalues[j]; // @OGUZ-BEWARE
